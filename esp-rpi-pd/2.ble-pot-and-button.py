    ...
    try:
        ...
        if (ch.supportsRead()):
            while 1:
                ...
                potVal = struct.unpack('<hB', val)[0]
                buttonVal = struct.unpack('<hB', val)[1]
                print "button: ", buttonVal, "   potentiometer: ", str(potVal)
                sendMsg(potVal)
                ledVal = potVal / 4095.0 # we are receiving 0 to 4095 (12-bits)
                led.value = ledVal
